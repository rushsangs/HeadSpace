...
namespace NarrativePlanning
{
    ...
    public class PlanningProblem
    {
        ...
        public Plan FFApparentSolution()
        {
            ...
            while (solutionPlan == null)
            {
                ...
                foreach (Tuple<String, WorldState> next in n)
                {
                    ...
                    Console.Write("Possible apparent next step " + next.Item1 + " and character heuristic of " + y + "\n");
                    if (y < min && y != -1)
                    {
                        best = res;
                        min = y;
        ...
        public Plan HeadSpaceXSolution()
        {
            ...
            return HeadSpaceX(this.desires, current, this.goal, groundedoperators);
        ...
        public Plan HeadSpaceX(List<Desire> desires, Plan plan, WorldState goal, List<Operator> groundedoperators)
        {
            ...
            foreach(Intention i in intentions)
            {
                if (!Intention.containsIntention(w.intentions, i))
                {
                    ...
                    if (p != null)
                    {
                        ...
                        i.plan.computeCLinks(pp.groundedoperators);
                        w.intentions.Add(i);
            ...
            if (possibleSteps.Count > 0)
            {
                ...
                {
                    if (i.plan.steps[i.plan.executed.Count].Equals(step.Item1))
                        i.plan.executed.Add(true);
                }
                //possibleSteps[selectedindex].Item2.plan.executed.Add(true);
                //plan.computeCLinks();
                    ...
                    else
                    {
                        if(CausalLink.isLinkThreatened(intention.plan, newState))
                        {
                            ...
                            else
                            {
                                ...
                                intention.plan.computeCLinks(pp.groundedoperators);
                            }
                        }
